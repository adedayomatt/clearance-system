    ...
    <div class="card-body">
        <div class="d-flex">
            ...
            <div class="ml-1">
                ...
                @auth('web')
                    <strong class="d-block text-muted">{{$clearance->student->prospect->fullname}}</strong>
                @endauth
                <div>
                    <small class="text-muted">{{$clearance->student->prospect->level}}L, {{$clearance->created_at->format('d F, Y h:i a')}}, {{$clearance->created_at->diffForHumans()}}</small>
                </div>
